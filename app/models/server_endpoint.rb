class ServerEndpoint
  ...
  one :server_endpoint_log, :dependent => :destroy, :autosave => false
  one :server_endpoint_remote_log, :dependent => :destroy, :autosave => false
  one :deploy_server_endpoint_job, :dependent => :destroy, :autosave => false

  attr_accessible :name, :endpoint_type, :remote_name, :backend, :backend_id, :master_slug, :user_name, :n_servers
  ...
  def destroy_app
    if deploy_server_endpoint_job
      deploy_server_endpoint_job.destroy_remote_endpoint
    end
  end
  ...
  def log_endpoint
    if persisted?
      begin
        @endpoint = ServerEndpoint.find(self.id)
        ...
        if @endpoint.server_endpoint_log
  ...
  def log_save_backtrace
    ...
    if @endpoint && @endpoint.server_endpoint_log
  ...
  def job_status
    deploy_server_endpoint_job.get_status if deploy_server_endpoint_job
  end
  ...
  def self.new_instance_for_backend(backend, endpoint_type = "Heroku")
    ...
    count = backend.server_endpoints.count
    remote_name = "busme-#{count}-#{name.gsub(".", "-")}"[0..29]
    endpoint = ServerEndpoint.new(:name => remote_name,
  ...
  def logger
    ...
    if self.server_endpoint_log.nil?
      self.create_server_endpoint_log
    end
    @my_logger           = MyLogger.new(self.server_endpoint_log)
    @my_logger.level     = self.log_level
