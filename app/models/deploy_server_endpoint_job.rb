    ...
    case server_endpoint.endpoint_type
      ...
      when "Unix"
        begin
          ...
          file.unlink
          log "#{head}: Configuration Result #{result.inspect}"
          result = Rush.bash unix_ssh_cmd("test -e buspass-web/script/configure_unix_server_endpoint.sh && bash --login -c 'cd buspass-web; script/configure_unix_server_endpoint.sh #{server_endpoint.name}' || exit 0")
          server_endpoint.reload
